...
/**
 * TEST VERSION v4: Mimics rev005 architecture for fixed RSSI
 * This version uses the same data flow as rev005:
 * - BLE Central generates simulated RSSI
 * - Calls mipe_rssi_callback
 * - Callback forwards to MotoApp
 * - No actual Mipe connection required
 */
...
int main(void)
{
    ...
    LOG_INF("SinglePing Host Device TEST VERSION v4 - Rev005 Architecture");
    LOG_INF("=============================================================");
    ...
    LOG_INF("TEST MODE v4: Using rev005 architecture");
    LOG_INF("Will generate fixed RSSI = -55 dBm via BLE Central callback");
    LOG_INF("LED0: Heartbeat, LED1: MotoApp, LED3: Streaming activity");
...
static void motoapp_connection_callback(bool connected)
{
    ...
    if (connected) {
        ...
        /* In test mode, we don't actually scan but we trigger the 
         * simulated RSSI generation by "pretending" to connect to Mipe
         */
        LOG_INF("TEST MODE: Simulating Mipe connection to start RSSI generation");
        mipe_connected = true;  /* Fake connection */
        gpio_pin_set_dt(&led2, 1); /* Show "connected" to Mipe */
        
        /* The BLE central's RSSI timer will start automatically
         * when it thinks it's connected (see ble_central.c)
         */
    } else {
...
static void data_stream_callback(bool start)
{
    ...
    if (start) {
        ...
        LOG_INF("TEST MODE: RSSI callbacks should start arriving from BLE Central");
    } else {
...
static void mipe_connection_callback(bool connected)
{
    /* In test mode, this is controlled by motoapp_connection_callback */
...
static void mipe_rssi_callback(int8_t rssi, uint32_t timestamp)
{
    ...
    /* Forward RSSI to MotoApp (same as rev005) */
    if (motoapp_connected && data_streaming && ble_peripheral_is_connected()) {
        ...
        if (err == 0) {
            ...
            LOG_INF("Forwarded simulated RSSI: %d dBm (packet %u) - Next in 2s", 
                    rssi, packet_count);
        } else {
