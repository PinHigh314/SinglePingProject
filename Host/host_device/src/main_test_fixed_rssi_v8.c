...
/**
 * Host Device Test Application - v8
...
LOG_MODULE_REGISTER(host_main_v8, LOG_LEVEL_INF);
...
static void app_connected(void)
{
    ...
    /* Update LED1 based on connection state */
    if (mipe_connected) {
...
static void app_disconnected(void)
{
    ...
    /* Stop rapid flash and turn off LED1 */
    k_timer_stop(&led1_rapid_flash_timer);
...
int main(void)
{
    ...
    LOG_INF("=== Host Device v8 Starting ===");
    LOG_INF("Features: Alternating RSSI + Real Mipe Connection");
    LOG_INF("LED1 rapid flash when dual connected");
