...
echo ========================================
echo SinglePing TestHost Device Build & Flash Script
echo Using stable cmake/ninja build process
echo ========================================
echo.
echo Usage: build-flash-testhost.bat [options] [description]
echo   Options:
echo     --clean or -c : Clean build (removes build directory first)
echo   Description:
echo     Optional description for the output hex file
echo     Example: build-flash-testhost.bat ble_central_test
echo     Example: build-flash-testhost.bat --clean initial_build
echo.
...
:: Navigate to TestHost directory (from BAT files folder)
...
echo.
echo Starting automatic flash operation...
echo Flashing device with: nrfjprog --program build\zephyr\zephyr.hex --chiperase --verify -r
nrfjprog --program build\zephyr\zephyr.hex --chiperase --verify -r

if %errorlevel% equ 0 (
    echo.
    echo Flash successful!
    echo Device is now programmed and ready
) else (
    echo.
    echo Flash failed! Please check:
    echo   1. Device is connected properly
    echo   2. nRF Connect Programmer tools are installed
    echo   3. Device is not in use by another program
    pause
    exit /b 1
)
...
if %errorlevel% equ 0 (
    ...
    :: Update version log
    echo %date% %time% - Built and flashed: %output_name% >> "%~dp0..\compiled_code\version_log.md"
) else (
