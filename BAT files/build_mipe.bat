...
echo ========================================
echo.
echo Usage: build_mipe.bat [options] [description]
echo   Options:
echo     --clean or -c : Clean build (removes build directory first)
echo   Description:
echo     Optional description for the output hex file
echo     Example: build_mipe.bat max_tx_power
echo     Example: build_mipe.bat --clean test_rssi
echo.
...
:: Ask user if they want to flash the device
set /p flash_choice="Do you want to flash the Mipe device now? (Y/N): "
...
if /i "%flash_choice%"=="Y" (
    ...
    if %errorlevel% equ 0 (
        ...
        echo.
        echo ========================================
        echo Flash failed! Please check your device connection.
        echo ========================================
        echo.
        echo To manually flash later, use:
        echo   nrfjprog --program build\zephyr\zephyr.hex --chiperase --verify -r
    )
) else (
    echo.
    echo Skipping flash. To flash the device later, use:
    echo   nrfjprog --program build\zephyr\zephyr.hex --chiperase --verify -r
