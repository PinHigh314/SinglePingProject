...
/**
 * TEST VERSION v2: Fixed RSSI transmission for debugging
 * This version sends fixed RSSI = -55 dBm to test data flow
 * - Fixed LED3 to turn off when streaming stops
 * - Fixed RSSI data transmission
 */
...
int main(void)
{
    ...
    LOG_INF("SinglePing Host Device TEST VERSION v2 - Fixed RSSI");
    ...
    LOG_INF("TEST MODE v2: Will send fixed RSSI = -55 dBm when streaming starts");
...
static void data_timer_handler(struct k_timer *timer)
{
    if (motoapp_connected && data_streaming && ble_peripheral_is_connected()) {
        ...
        LOG_INF("Attempting to send RSSI data...");
        LOG_INF("  - Connected: %s", motoapp_connected ? "YES" : "NO");
        LOG_INF("  - Streaming: %s", data_streaming ? "YES" : "NO");
        LOG_INF("  - BLE Connected: %s", ble_peripheral_is_connected() ? "YES" : "NO");
        LOG_INF("  - RSSI Value: %d dBm", fixed_rssi);
        ...
        if (err == 0) {
            packet_count++;
            last_packet_time = k_uptime_get();
            LOG_INF("SUCCESS: Sent fixed RSSI: %d dBm (packet %u)", fixed_rssi, packet_count);
        } else {
        ...
        LOG_DBG("Skipping RSSI send - conditions not met");
    }
}
...
static void monitor_timer_handler(struct k_timer *timer)
{
    ...
    if (data_streaming) {
        ...
        int64_t time_since_last = now - last_packet_time;
        LOG_INF("Streaming status: Active, Last packet: %lld ms ago", time_since_last);
    }
}
